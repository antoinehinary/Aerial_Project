...
class Agent():
    ...
    def __init__(self, sensor_data, dt):
        ...
        self.map = np.zeros((int((self.max_x-self.min_x)/self.res_pos), int((self.max_y-self.min_y)/self.res_pos)))
        self.t = 0
    ...
    # def snake_return(self):

    #     goal_list = []
    #     for i in range(4):
    #         if i % 2 == 1:               # must be 4 to be correct
    #             for j in range(2):
    #                 goal_list.append(np.array([self.pos[0]+i*0.2,self.pos[1]-j]))
    #         if i % 2 == 0:
    #             for j in range(2):
    #                 goal_list.append(([self.pos[0]+i*0.2,self.pos[1]+j]))
    ...
    def occupancy_map(self, map):
        ...
        if self.t % 50 == 0:
            ...
            # plt.savefig('map.png')
            # plt.close()
            plt.imshow(np.flip(self.map, 1), vmin=-1, vmax=1, cmap='gray',
                       origin='lower')  # flip the map to match the coordinate system
            for cell in self.goal_list_grid:
                plt.plot(len(self.map[0])-1 -
                         cell[1], cell[0], 'o', color='orange')
            plt.scatter([len(self.map[0])-1 - ((self.pos[1]-self.min_y)/self.res_pos)],
                        [(self.pos[0] - self.min_x)/self.res_pos], color='red', marker='x', s=100)
            plt.savefig("map.png")
            plt.close()
        self.t += 1
    ...
    def go_to(self):
        ...
        if self.state != GO_HOME:
            if d < 0.3 and min(self.sensor_data["range.right"], self.sensor_data["range.left"], self.sensor_data["range.back"]) < 500:
                print("goal skiped")
                print("minuimum: ",  min(self.sensor_data["range.right"],
                      self.sensor_data["range.left"], self.sensor_data["range.back"]))
